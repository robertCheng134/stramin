...
from health_data import from_garmin_row
...
def _read_valid_garmin_rows(path):
    if not path.exists():
    ...
    with path.open(newline="", encoding="utf-8") as csv_file:
        ...
        rows = []
        for row in reader:
            normalized_row = {field: row.get(field, "").strip() for field in fields}
...
def load_health_data(csv_path=None):
    source_info = resolve_garmin_health_csv(csv_path)
    rows = _read_valid_garmin_rows(source_info["path"])
    return [from_garmin_row(row) for row in rows]


def load_garmin_health_rows(csv_path=None):
    return [health_data.to_legacy_dict() for health_data in load_health_data(csv_path)]
